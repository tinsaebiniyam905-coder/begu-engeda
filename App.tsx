...
export default function App() {
  ...
  const handleSignup = (e: React.FormEvent) => {
    e.preventDefault();
    if (loginData.password !== loginData.confirmPassword) return alert("Passwords do not match!");
    ...
    alert(`Confirmation number sent to ${loginData.identifier}: ${otp}`); // Simulated SMS
    setAuthState('otp');
  };
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (loginData.identifier === 'police' && loginData.password === '1234') {
      const pAcc = { username: 'Police_Office', role: UserRole.LOCAL_POLICE, isVerified: true, isProfileComplete: !!policeProfile.name } as any;
      setCurrentUser(pAcc);
  ...
  const handleLogout = () => { setCurrentUser(null); setAuthState('login'); setView('dashboard'); };
  ...
  if (authState !== 'authenticated') {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 font-sans">
        <div className="bg-white rounded-[2.5rem] shadow-2xl p-8 sm:p-12 w-full max-w-md animate-in zoom-in-95 duration-500">
          <img src={LOGO_PATH} className="w-24 h-24 mx-auto mb-6" onError={(e) => { e.currentTarget.src = "https://img.icons8.com/color/512/police-badge.png" }} />
          ...
          {authState === 'login' && (
            <form onSubmit={handleLogin} className="space-y-5">
              <Input label={t.username + " / " + t.phoneNumber} value={loginData.identifier} onChange={(v:any) => setLoginData({...loginData, identifier: v})} required icon={<UserCircle size={18}/>} />
              ...
              <button className="w-full bg-slate-800 hover:bg-slate-700 text-white font-black py-4 rounded-2xl shadow-xl transition-all uppercase text-sm mt-4">{t.login}</button>
              <button type="button" onClick={() => setAuthState('signup')} className="w-full text-center text-xs font-black text-indigo-600 uppercase mt-4">{t.noAccount}</button>
            </form>
          )}
          ...
          {authState === 'signup' && (
            <form onSubmit={handleSignup} className="space-y-5">
              ...
              <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-4 rounded-2xl shadow-xl transition-all uppercase text-sm mt-4">{t.signup}</button>
              <button type="button" onClick={() => setAuthState('login')} className="w-full text-center text-xs font-black text-indigo-600 uppercase mt-4">{t.alreadyHaveAccount}</button>
            </form>
          )}

          {authState === 'otp' && (
            <form onSubmit={handleVerifyOtp} className="space-y-6">
              <p className="text-center font-bold text-slate-600 text-sm">{t.enterOtp}</p>
              <Input label={t.otpLabel} value={loginData.otp} onChange={(v:any) => setLoginData({...loginData, otp: v})} required icon={<ShieldCheck size={18}/>} />
              <button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-2xl shadow-xl transition-all uppercase text-sm">{t.verify}</button>
            </form>
          )}

          {authState === 'setup_hotel' && (
            <form onSubmit={handleHotelSetup} className="space-y-5">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter text-center">{t.setupHotel}</h3>
              <Input label={t.hotel} value={hotelProfile.name} onChange={(v:any) => setHotelProfile({...hotelProfile, name: v})} required icon={<Building2 size={18}/>} />
              <Input label={t.hotelAddress} value={hotelProfile.address} onChange={(v:any) => setHotelProfile({...hotelProfile, address: v})} required icon={<MapPin size={18}/>} />
              <div className="space-y-2">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[3px] ml-1">{t.zone}</label>
                <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 font-black text-sm outline-none" value={hotelProfile.zone} onChange={e => setHotelProfile({...hotelProfile, zone: e.target.value})} required>
                  <option value="">Select Zone</option>
                  {ZONES.map(z => <option key={z} value={z}>{z}</option>)}
                </select>
              </div>
              <button className="w-full bg-slate-900 text-white font-black py-5 rounded-2xl uppercase tracking-widest">{t.save} & {t.submit}</button>
            </form>
          )}

          {authState === 'setup_police' && (
            <form onSubmit={handlePoliceSetup} className="space-y-5">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter text-center">{t.setupPolice}</h3>
              <Input label={t.policeOfficeName} value={policeProfile.name} onChange={(v:any) => setPoliceProfile({...policeProfile, name: v})} required icon={<ShieldCheck size={18}/>} />
              <Input label={t.location} value={policeProfile.address} onChange={(v:any) => setPoliceProfile({...policeProfile, address: v})} required icon={<MapPin size={18}/>} />
              <div className="space-y-2">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[3px] ml-1">{t.zone}</label>
                <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 font-black text-sm outline-none" value={policeProfile.zone} onChange={e => setPoliceProfile({...policeProfile, zone: e.target.value})} required>
                  <option value="">Select Jurisdiction</option>
                  {ZONES.map(z => <option key={z} value={z}>{z}</option>)}
                </select>
              </div>
              <button className="w-full bg-slate-900 text-white font-black py-5 rounded-2xl uppercase tracking-widest">{t.save}</button>
            </form>
          )}

          <div className="mt-10 flex justify-center gap-6 pt-6 border-t border-slate-100">
            <button onClick={() => setLang('am')} className={`px-4 py-2 rounded-xl text-[10px] font-black ${lang === 'am' ? 'bg-amber-500 text-white' : 'bg-slate-100'}`}>አማርኛ</button>
            <button onClick={() => setLang('en')} className={`px-4 py-2 rounded-xl text-[10px] font-black ${lang === 'en' ? 'bg-amber-500 text-white' : 'bg-slate-100'}`}>ENGLISH</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] flex flex-col md:flex-row font-sans text-slate-900">
      {isSidebarOpen && <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setIsSidebarOpen(false)} />}
      
      {/* Sidebar - Pro Desktop / Responsive Mobile */}
      <aside className={`fixed md:relative z-50 w-72 h-full bg-[#111827] text-white flex flex-col transition-all duration-300 no-print 
        ...
        <div className="p-8 border-b border-white/5 text-center flex flex-col items-center">
          <img src={LOGO_PATH} className="w-16 h-16 mb-4" onError={(e) => { e.currentTarget.src = "https://img.icons8.com/color/512/police-badge.png" }} />
          <h2 className={`text-xl leading-tight ${GOLDEN_GRADIENT}`}>{t.appName}</h2>
          <p className="text-[9px] font-black text-gray-400 uppercase mt-2 tracking-[1px]">{t.developedBy}</p>
        </div>
        ...
        <div className="p-6 bg-[#030712] border-t border-white/5">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="bg-white border-b px-6 py-5 flex justify-between items-center shadow-sm z-30">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h3 className="font-black text-slate-800 uppercase text-sm sm:text-lg tracking-tighter">{t[view] || view}</h3>
               <p className="text-[10px] text-slate-400 font-bold uppercase hidden sm:block">
                 {currentUser?.role === UserRole.RECEPTION ? hotelProfile.name : policeProfile.name || "Jurisdiction Command"}
               </p>
             </div>
          </div>
          <div className="flex items-center gap-4">
             <button onClick={() => setLang(lang === 'am' ? 'en' : 'am')} className="p-3 bg-slate-50 border rounded-xl hover:bg-amber-50 transition-all"><Globe size={20} className="text-amber-600" /></button>
             <div className="flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-100 rounded-2xl">
                <div className="text-right leading-tight hidden xs:block">
                   <p className="text-xs font-black text-slate-900 uppercase">{currentUser?.username}</p>
                   <p className="text-[9px] text-amber-600 font-black uppercase">{currentUser?.role}</p>
                </div>
        ...
        <main className="flex-1 overflow-y-auto p-5 sm:p-10 custom-scrollbar">
          ...
          <div className="max-w-7xl mx-auto animate-in fade-in duration-700">
            ...
            {view === 'registerGuest' && <GuestForm newGuest={newGuest} setNewGuest={setNewGuest} onSubmit={saveGuest} t={t} />}
              ...
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 no-print">
                   <div className="relative w-full max-w-md">
                     ...
                     <input type="text" placeholder={t.searchPlaceholder} className="w-full bg-white border-2 border-slate-100 rounded-2xl pl-14 pr-6 py-4 outline-none focus:border-amber-400 font-bold shadow-sm transition-all" 
                       value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                   </div>
                   <div className="flex gap-3 w-full sm:w-auto">
                      <button onClick={() => window.print()} className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 py-4 bg-white border-2 border-slate-100 rounded-2xl hover:bg-slate-50 transition-all font-black uppercase text-[10px] tracking-widest shadow-md"><Printer size={18}/> {t.print}</button>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden">
                  <div className="p-10 border-b bg-slate-50/50">
                    <h3 className="text-2xl font-black text-slate-800 tracking-tighter uppercase">{t.fullTableRecord}</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Comprehensive Regional Surveillance Log</p>
                  </div>
                  <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left whitespace-nowrap">
                      <thead className="bg-slate-50 text-[11px] font-black uppercase text-slate-400 tracking-[3px] border-b border-slate-100">
                        <tr>
                          <th className="px-12 py-10">ID Evidence</th>
                          <th className="px-12 py-10">Full Name</th>
                          <th className="px-12 py-10">Room No.</th>
                          <th className="px-12 py-10">Property Details</th>
                          <th className="px-12 py-10">Verification Profile</th>
                          <th className="px-12 py-10 text-center">Protocol Status</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 text-[12px] font-bold uppercase text-slate-700">
                        {filteredGuests.map((g: any) => (
                          <tr key={g.id} className="hover:bg-indigo-50/50 transition-all duration-300 group">
                            <td className="px-12 py-8">
                              <div className="w-24 h-32 rounded-2xl overflow-hidden shadow-2xl ring-4 ring-white border border-slate-100 transform group-hover:scale-105 transition-transform cursor-zoom-in" onClick={() => setZoomImg(g.idPhoto)}>
                            ...
                            <td className="px-12 py-8">
                              <span className="inline-block px-6 py-3 bg-slate-100 rounded-2xl text-slate-800 font-black border border-slate-200 shadow-inner text-lg">#{g.roomNumber}</span>
                            </td>
                            <td className="px-12 py-8 leading-relaxed">
                              <p className="text-slate-800 text-base font-black tracking-tight">{g.hotelName}</p>
                              <p className="text-[11px] text-slate-400 mt-2 font-black">{g.hotelAddress} • {g.hotelZone}</p>
                            </td>
                            <td className="px-12 py-8">
                              <p className="text-slate-700 text-sm">{g.receptionistName}</p>
                              <div className="flex items-center gap-2 text-[10px] text-indigo-500 font-black mt-3">
                                <Phone size={14}/> {g.receptionistPhone}
                              </div>
                            </td>
                            <td className="px-12 py-8 text-center">
                              {g.isWanted ? (
                                <span className="inline-flex items-center gap-3 px-8 py-4 bg-red-600 text-white rounded-full text-[11px] font-black shadow-xl animate-pulse border-2 border-white">
                                  <AlertTriangle size={16}/> INTERCEPT REQUIRED
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-3 px-8 py-4 bg-emerald-500 text-white rounded-full text-[11px] font-black shadow-lg border-2 border-white">
                                  <CheckCircle2 size={16}/> CLEARANCE GRANTED
                                </span>
                              )}
                            </td>
                          </tr>
                        ...
                        {filteredGuests.length === 0 && (
                          <tr><td colSpan={6} className="p-40 text-center text-slate-300 font-black tracking-[10px] uppercase opacity-40">No Regional Records Found</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
            ...
            {view === 'settings' && (
              <div className="max-w-xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100">
                <h3 className="text-2xl font-black mb-8 uppercase tracking-tighter text-center">{t.settings}</h3>
                <div className="space-y-6">
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Account Profile</p>
                    <p className="font-bold text-slate-800">{currentUser?.username}</p>
                    <p className="text-xs text-slate-500">{currentUser?.phoneNumber}</p>
                  </div>
                  <button className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-xl transition-all uppercase tracking-widest">{t.save} Changes</button>
                </div>
              </div>
            )}
          </div>
...
// Fixed: Only one implementation of NavItem
function NavItem({ icon, label, active, onClick, count }: any) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[2px] transition-all duration-300 group
      ...
      {count > 0 && <span className="bg-red-500 text-white text-[9px] px-2.5 py-1 rounded-full font-black shadow-lg ring-4 ring-[#111827]">{count}</span>}
    </button>
  );
}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {stats.map(s => (
          <div key={s.l} className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer overflow-hidden relative" 
            ...
            <div className={`${s.c} w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-white shadow-2xl transition-all duration-500 group-hover:rotate-12`}>
              {s.icon}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl border border-slate-100 h-[450px]">
          <h4 className="font-black text-slate-800 uppercase mb-10 text-xs tracking-[4px] flex items-center gap-4">
            <TrendingUp size={24} className="text-indigo-500"/> Surveillance Insights
          </h4>
        ...
        <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl border border-slate-100 flex flex-col h-[450px]">
          <h4 className="font-black text-slate-800 uppercase mb-8 text-xs tracking-[4px] flex items-center gap-4">
            <Activity size={24} className="text-emerald-500"/> Judicial Feed
          </h4>
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            <table className="w-full text-left text-[11px] font-black">
              <thead className="bg-slate-50 uppercase text-slate-400 sticky top-0">
                <tr><th className="p-4">Name</th><th className="p-4">Zone</th><th className="p-4 text-center">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {guests.slice(0, 10).map(g => (
                  <tr key={g.id} className="hover:bg-indigo-50/50 transition-colors duration-300">
                    <td className="p-4 uppercase text-slate-700">{g.fullName}</td>
                    <td className="p-4 uppercase text-slate-400">{g.hotelZone}</td>
                    <td className="p-4 text-center">
                      {g.isWanted ? <span className="text-red-600 font-bold">WANTED</span> : <span className="text-emerald-600">SECURE</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function GuestForm({ onSubmit, newGuest, setNewGuest, t }: any) {
        ...
        <div className="space-y-4">
          ...
          <div className="p-16 bg-slate-50/50 border-4 border-dashed border-slate-100 rounded-[3rem] text-center cursor-pointer hover:bg-slate-50 hover:border-indigo-400 transition-all duration-500 group" onClick={() => document.getElementById('idUpload')?.click()}>
            <div className="flex flex-col items-center">
              <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center shadow-2xl mb-6 text-slate-200 group-hover:text-indigo-600 transition-all border-4 border-white"><Camera size={40}/></div>
...
function UtilitySection({ t }: any) {
  return (
    <div className="bg-white p-12 sm:p-20 rounded-[5rem] shadow-2xl border border-slate-100 space-y-12 max-w-5xl mx-auto text-center">
      <div className="flex flex-col items-center">
        <img src={LOGO_PATH} className="w-40 h-40 mb-10 drop-shadow-2xl grayscale opacity-20" onError={(e) => { e.currentTarget.src = "https://img.icons8.com/color/512/police-badge.png" }} />
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-12 duration-700">
      {notifications.map((n: any) => (
        <div key={n.id} className={`p-10 bg-white border-l-[15px] rounded-[3rem] shadow-2xl flex gap-10 transition-all hover:-translate-x-2 
          ...
          <div className={`w-20 h-20 rounded-[1.5rem] shadow-2xl border-4 border-white flex-shrink-0 flex items-center justify-center 
          ...
          <div className="flex-1">
            ...
            {n.guestId && (
              <button onClick={() => setView('guestList')} className="mt-8 px-10 py-4 bg-red-600 hover:bg-red-700 text-white text-xs font-black uppercase rounded-2xl shadow-2xl shadow-red-100 transition-all tracking-[4px]">Access Record</button>
            )}
          </div>
        </div>
      ))}
      {notifications.length === 0 && (
        <div className="text-center py-60 opacity-20 flex flex-col items-center">
          <ShieldCheck size={120} className="mb-10 text-slate-200" />
          <h3 className="text-2xl font-black uppercase tracking-[15px] text-slate-300">System Secure</h3>
        </div>
      )}
    </div>
  );
